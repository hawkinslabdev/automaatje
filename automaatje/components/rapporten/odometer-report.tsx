...
type SortField = "date" | "vehicle" | "odometer" | "distance";
type SortDirection = "asc" | "desc";

export function OdometerReport({
  data,
  stats,
  vehicles,
  organizations,
  ...
  const handlePeriodChange = (period: string) => {
    ...
    router.push(`/rapporten/kilometers?${params.toString()}`);
  };
  ...
  const handleExportPDF = async (vehicleId: string, organizationId: string) => {
    ...
    try {
      ...
      // Call server action
      const result = await exportOdometerReportPDF({
        startDate,
        endDate,
      ...
      if (!result.success || !result.data) {
        toast({
          title: "Fout bij exporteren",
          description: result.error || "Kon PDF niet genereren",
          variant: "destructive",
        });
        return;
      }
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={currentTripType} onValueChange={handleTripTypeChange}>
                <SelectTrigger>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        {/* Show on mobile horizontally */}
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Totaal afstand</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Privé kilometers</CardTitle>
            <TrendingUp className="h-4 w-4 text-muted-foreground shrink-0" />
          </CardHeader>
          <CardContent>
            <div className="text-xl lg:text-2xl font-bold">{formatDistance(stats.privateKilometers || 0)}</div>
            <p className="text-xs text-muted-foreground">
              Privé gereden kilometers
            </p>
          </CardContent>
        </Card>
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <div className="flex gap-3 text-sm text-muted-foreground">
                        ...
                        {vehicleStats.privateKilometers !== undefined && vehicleStats.privateKilometers > 0 && (
                          <>
                            <span>•</span>
                            <span>
                              {formatDistance(vehicleStats.privateKilometers)} privé
                            </span>
                          </>
                        )}
                      </div>
        ...
        <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <CardDescription className="text-sm">
              Alle kilometerstandregistraties in de geselecteerde periode
            </CardDescription>
          </div>
          ...
          {!hasData ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground max-w-sm">
                Er zijn geen kilometerstandregistraties in de geselecteerde periode.
              </p>
            </div>
          ) : (
                  ...
                  {sortedData.map((item) => {
                    ...
                    return (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="text-right font-mono">
                          {item.parsedData.endOdometerKm ? `${item.parsedData.endOdometerKm.toLocaleString("nl-NL")} km` : "-"}
                        </TableCell>
                        <TableCell className="text-right font-mono">
                          {item.parsedData.distanceKm
                            ? formatDistance(item.parsedData.distanceKm)
                            : "-"}
                        </TableCell>
                        <TableCell>
                          {item.parsedData.tripType ? (
